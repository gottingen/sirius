...
namespace sirius {
    ...
    turbo::Status DiscoverySender::init(const std::vector<std::string> & raft_nodes) {
        _master_leader_address.ip = mutil::IP_ANY;
        std::vector<std::string> peers = raft_nodes;
        for (auto &peer : peers) {
    ...
    turbo::Status DiscoverySender::discovery_naming(const sirius::proto::DiscoveryQueryRequest &request,
                                                    sirius::proto::DiscoveryQueryResponse &response, int retry_time) {
        return send_request("discovery_query", request, response, retry_time);
    }
