...
namespace sirius::discovery {
    ...
    int SchemaManager::check_and_get_for_privilege(sirius::proto::UserPrivilege &user_privilege) {
        ...
        if (namespace_id == 0) {
            SS_LOG(ERROR) << "namespace not exist, namespace:" << namespace_name
        ...
        for (auto &pri_servlet: *user_privilege.mutable_privilege_servlet()) {
            ...
            int64_t zone_id = ZoneManager::get_instance()->get_zone_id(base_name);
            if (zone_id == 0) {
                SS_LOG(ERROR) << "zone: " << base_name << " not exist, namespace: " << namespace_name
                              << ", request: " << user_privilege.ShortDebugString();
                return -1;
            }
    ...
    int SchemaManager::check_and_get_for_instance(sirius::proto::ServletInstance &instance) {
        std::string namespace_name = instance.namespace_name();
        int64_t namespace_id = NamespaceManager::get_instance()->get_namespace_id(namespace_name);
        if (namespace_id == 0) {
            SS_LOG(ERROR) << "namespace not exist, namespace:" << namespace_name
                          << ", request:" << instance.ShortDebugString();
            return -1;
        }
        ...
        int64_t servlet_id = ServletManager::get_instance()->get_servlet_id(servlet_name);
        if (servlet_id == 0) {
            SS_LOG(ERROR) << "servlet: " << servlet_name << " not exist, zone: " << base_name
                          << ", namespace: " << namespace_name
                          << ", request: " << instance.ShortDebugString();
            return -1;
        }
    ...
    int SchemaManager::load_max_id_snapshot(const std::string &max_id_prefix,
        ...
        if (max_key == DiscoveryConstants::MAX_NAMESPACE_ID_KEY) {
            ...
            SS_LOG(WARN) << "max_namespace_id:" << *max_id;
            return 0;
        }
