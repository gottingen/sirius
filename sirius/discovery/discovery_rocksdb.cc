...
namespace sirius::discovery {
    ...
    int DiscoveryRocksdb::init() {
        ...
        if (!_rocksdb) {
            SS_LOG(ERROR) << "create rocksdb handler failed";
            return -1;
        }
        int ret = _rocksdb->init(FLAGS_discovery_db_path);
        if (ret != 0) {
            SS_LOG(ERROR) << "rocksdb init failed: code: " << ret;
            return -1;
        }
        _handle = _rocksdb->get_meta_info_handle();
        SS_LOG(WARN) << "rocksdb init success, db_path: " << FLAGS_discovery_db_path;
        return 0;
    }
    ...
    int DiscoveryRocksdb::put_discovery_info(const std::string &key, const std::string &value) {
        ...
        if (!status.ok()) {
            SS_LOG(WARN) << "put rocksdb fail, err_msg: " << status.ToString()<<", key: "<<key<<", value: "<<value;
            return -1;
        }
        return 0;
    }
    ...
    int DiscoveryRocksdb::put_discovery_info(const std::vector<std::string> &keys,
        ...
        if (keys.size() != values.size()) {
            SS_LOG(WARN) << "input keys'size is not equal to values' size";
            return -1;
        }
        rocksdb::WriteOptions write_option;
        write_option.disableWAL = true;
        ...
        if (!status.ok()) {
            SS_LOG(WARN) << "put batch to rocksdb fail, err msg: " << status.ToString();
            return -1;
        }
        return 0;
    }
    ...
    int DiscoveryRocksdb::remove_discovery_info(const std::vector<std::string> &keys) {
        ...
        auto status = _rocksdb->write(write_option, &batch);
        if (!status.ok()) {
            SS_LOG(WARN) << "delete batch to rocksdb fail,  " << status.ToString();
            return -1;
        }
        return 0;
    }
    ...
    int DiscoveryRocksdb::write_discovery_info(const std::vector<std::string> &put_keys,
        ...
        auto status = _rocksdb->write(write_option, &batch);
        if (!status.ok()) {
            SS_LOG(WARN) << "write batch to rocksdb fail,  " << status.ToString();
            return -1;
        }
        return 0;
    }
