...
namespace sirius {
    ...
    int32_t RocksStorage::init(const std::string &path) {
        ...
        std::shared_ptr<mizar::EventListener> my_listener = std::make_shared<SimpleListener>();
        mizar::BlockBasedTableOptions table_options;
        if (FLAGS_rocks_use_partitioned_index_filters) {
            // use Partitioned Index Filters
            // https://github.com/facebook/rocksdb/wiki/Partitioned-Index-Filters
            table_options.index_type = mizar::BlockBasedTableOptions::kTwoLevelIndexSearch;
...
#if ROCKSDB_MAJOR == 7
                auto cache_opt = mizar::HyperClockCacheOptions(FLAGS_rocks_block_cache_size_mb * 1024 * 1024LL, FLAGS_rocks_block_size, 8);
                cache_opt.metadata_charge_policy = mizar::kDontChargeCacheMetadata; //cache会比rocks_block_cache_size_mb多占用少量内存
                table_options.block_cache = cache_opt.MakeSharedCache();
#endif
            } else {
                table_options.block_cache = mizar::NewLRUCache(FLAGS_rocks_block_cache_size_mb * 1024 * 1024LL, 8);
            }
        }
        ...
        if (FLAGS_rocks_use_ribbon_filter) {
            table_options.filter_policy.reset(mizar::NewRibbonFilterPolicy(9.9));
        } else {
            table_options.filter_policy.reset(mizar::NewBloomFilterPolicy(10));
        }
        _cache = table_options.block_cache.get();
        mizar::Options db_options;
        ...
        if (FLAGS_rocks_kSkipAnyCorruptedRecords) {
            db_options.wal_recovery_mode = mizar::WALRecoveryMode::kSkipAnyCorruptedRecords;
        }
        db_options.statistics = mizar::CreateDBStatistics();
        db_options.max_subcompactions = FLAGS_rocks_max_subcompactions;
        db_options.max_background_flushes = 2;
        db_options.env->SetBackgroundThreads(2, mizar::Env::HIGH);
        db_options.listeners.emplace_back(my_listener);
        mizar::TransactionDBOptions txn_db_options;
        ...
        txn_db_options.custom_mutex_factory = std::shared_ptr<mizar::TransactionDBMutexFactory>(
                new TransactionDBBthreadFactory());

        //todo
        _log_cf_option.prefix_extractor.reset(
                mizar::NewFixedPrefixTransform(sizeof(int64_t) + 1));
        _log_cf_option.OptimizeLevelStyleCompaction();
        _log_cf_option.compaction_pri = mizar::kOldestLargestSeqFirst;
        _log_cf_option.table_factory.reset(mizar::NewBlockBasedTableFactory(table_options));
        _log_cf_option.compaction_style = mizar::kCompactionStyleLevel;
        ...
        _data_cf_option.prefix_extractor.reset(
                mizar::NewFixedPrefixTransform(sizeof(int64_t) * 2));
        ...
        _data_cf_option.compaction_pri = static_cast<mizar::CompactionPri>(FLAGS_rocks_data_compaction_pri);
        if (FLAGS_rocks_use_sst_partitioner_fixed_prefix) {
            // 按region_id拆分
            _data_cf_option.sst_partitioner_factory = mizar::NewSstPartitionerFixedPrefixFactory(sizeof(int64_t));
        }
        _data_cf_option.table_factory.reset(mizar::NewBlockBasedTableFactory(table_options));
        _data_cf_option.compaction_style = mizar::kCompactionStyleLevel;
        ...
        if (FLAGS_l0_compaction_use_lz4) {
            _data_cf_option.compression_per_level = {mizar::CompressionType::kNoCompression,
                                                     mizar::CompressionType::kLZ4Compression,
                                                     mizar::CompressionType::kLZ4Compression,
                                                     mizar::CompressionType::kLZ4Compression,
                                                     mizar::CompressionType::kLZ4Compression,
                                                     mizar::CompressionType::kLZ4Compression,
        ...
        if (FLAGS_enable_bottommost_compression) {
            ...
            _data_cf_option.bottommost_compression = mizar::kZSTD;
        ...
        _meta_info_option.prefix_extractor.reset(
                mizar::NewFixedPrefixTransform(1));
        _meta_info_option.OptimizeLevelStyleCompaction();
        _meta_info_option.compaction_pri = mizar::kOldestSmallestSeqFirst;
        ...
        mizar::Status s;
        s = mizar::DB::ListColumnFamilies(db_options, path, &column_family_names);
        //db已存在
        if (s.ok()) {
            std::vector<mizar::ColumnFamilyDescriptor> column_family_desc;
            std::vector<mizar::ColumnFamilyHandle *> handles;
            for (auto &column_family_name: column_family_names) {
                if (column_family_name == RAFT_LOG_CF) {
                    column_family_desc.push_back(mizar::ColumnFamilyDescriptor(RAFT_LOG_CF, _log_cf_option));
                } else if (column_family_name == DATA_CF) {
                    column_family_desc.push_back(mizar::ColumnFamilyDescriptor(DATA_CF, _data_cf_option));
                } else if (column_family_name == META_INFO_CF) {
                    column_family_desc.push_back(mizar::ColumnFamilyDescriptor(META_INFO_CF, _meta_info_option));
                } else {
                    column_family_desc.push_back(
                            mizar::ColumnFamilyDescriptor(column_family_name,
                                                            mizar::ColumnFamilyOptions()));
                }
            }
            s = mizar::TransactionDB::Open(db_options,
                                             txn_db_options,
                                             path,
                                             column_family_desc,
                                             &handles,
            ...
            // new db
            s = mizar::TransactionDB::Open(db_options, txn_db_options, path, &_txn_db);
            if (s.ok()) {
        ...
        if (0 == _column_families.count(RAFT_LOG_CF)) {
            ...
            mizar::ColumnFamilyHandle *raft_log_handle;
        ...
        if (0 == _column_families.count(DATA_CF)) {
            ...
            mizar::ColumnFamilyHandle *data_handle;
        ...
        if (0 == _column_families.count(META_INFO_CF)) {
            mizar::ColumnFamilyHandle *metainfo_handle;
    ...
    mizar::Status RocksStorage::remove_range(const mizar::WriteOptions &options,
        ...
        if (delete_files_in_range && FLAGS_delete_files_in_range) {
            auto s = mizar::DeleteFilesInRange(_txn_db, column_family, &begin, &end, false);
            if (!s.ok()) {
                return s;
            }
        }
        mizar::TransactionDBWriteOptimizations opt;
    ...
    int32_t RocksStorage::delete_column_family(std::string cf_name) {
        ...
        mizar::ColumnFamilyHandle *cf_handler = _column_families[cf_name];
    ...
    int32_t RocksStorage::create_column_family(std::string cf_name) {
        ...
        mizar::ColumnFamilyHandle *cf_handler = nullptr;
    ...
    mizar::ColumnFamilyHandle *RocksStorage::get_raft_log_handle() {
        if (!_is_init) {
    ...
    mizar::ColumnFamilyHandle *RocksStorage::get_data_handle() {
        if (!_is_init) {
            LOG(ERROR) << "rocksdb has not been inited";
            return nullptr;
        }
    ...
    mizar::ColumnFamilyHandle *RocksStorage::get_meta_info_handle() {
        if (!_is_init) {
            LOG(ERROR) << "rocksdb has not been inited";
            return nullptr;
        }
